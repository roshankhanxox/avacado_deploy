...
export function Operations({
    ...
    const handleSubmitWithdrawIntent_ = async (
        amount: string,
        destination: string,
        nonce: string
    ) => {
        await handleSubmitWithdrawIntent(amount, destination, nonce);
        refetchBalance();
    };
    ...
    return (
        <div className="flex flex-col font-mono space-y-2">
            <p className="text-sm text-cyber-gray font-mono leading-relaxed mb-4 mt-2 indent-6">
                ...
                cryptography. When you mint, transfer, or burn tokens, the data
                is encrypted with your public key so that only you can decrypt
                and view your balances. For transfer operations, the recipient’s
                public key will be automatically fetched from the protocol to
                encrypt the tokens for them.
            </p>
            ...
            {mode === "converter" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="border border-cyber-green/30 bg-black/10 rounded-lg p-4 flex flex-col min-h-[200px] col-span-1 md:col-span-2">
                        <WithdrawIntent
                            handleSubmitWithdrawIntent={
                                handleSubmitWithdrawIntent_
                            }
                            handleExecuteWithdrawIntent={
                                handleExecuteWithdrawIntent
                            }
